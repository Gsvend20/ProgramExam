...
void spawnWalls() //Spawning turtles to control
{
  ...
  //Spawning first wall
  wall_spawn_msg.request.name = "turtle2"; // Declaring variables and giving them values 'VARIABLE InITILISER'
...
//Functioncalls to Draw the walls and stop drawing
  ...
  {
    key = getch();
    switch (key) //Beginning of a switch case, used to transfer control to one depending on the value of condition
    {
      case 119: //119 = w
        turtle2Twist.linear.x = 1; //Define linear.x as 1 for turtle2
        turtle2Move.publish(turtle2Twist); //And then publish this data for the tURTLE 2
        break;
